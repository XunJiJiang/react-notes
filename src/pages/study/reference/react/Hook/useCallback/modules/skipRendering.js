...
const Test = React.memo(function ({ testHandler, isUseCallback }) {
...
function SkipRendering () {
  ...
  const [b] = React.useState(new class B {
    b = 1
  }());
  const testHandler = (title) => {
    console.log(title, '重新渲染', ';use state:', b);
  };
  const testHandlerUseCallback = React.useCallback((title) => {
    console.log(title, '重新渲染', ';use state:', b);
  }, [b]);
